...
export async function DELETE(request, { params }) {
    ...
    if (!reto) return NextResponse.json({ error: "No encontrado" }, { status: 404 });
    if (!session.user.isAdmin && reto.grado !== session.user.grado) {
        return NextResponse.json({ error: "No autorizado para este grado" }, { status: 403 });
    }

    await prisma.reto.delete({ where: { id: params.id } });
